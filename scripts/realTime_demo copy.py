...
from fv_lib.flow_vis import flow_to_color
point_cont = 0
...
def put_optical_flow_arrows_on_image(image, optical_flow_image, threshold=1.2):
    ...
    scaled_flow = optical_flow_image * 10
...
def Imagecallback(msg):
    ...
    img = dst[y:y+h, x:x+w]
    flow = tracker.track(img.copy())
    (l,w) = img.shape
    # img = img[int(0.1*l):int(0.9*l),int(0.1*w):int(0.8*w)]
    flow = flow[int(0.1*l):int(0.9*l),int(0.1*w):int(0.8*w), :]
    s_w = 214*2
    s_l = 182*2
    flow = cv2.resize(flow.copy(), (s_w, s_l))
